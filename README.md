...
## 프로젝트 소개  
- SK Telecom 산학협력 인공지능 석학사 통합 교과목 진행 프로젝트입니다.  
- NUGU Play Kit Platform을 이용해서 진행되었으며, 유저 맞춤형 영화 추천 서비스 입니다.  
...
## 개발 동기  
- 혼영, 혼술 등의 여가활동을 집에서 즐기는 "홈스테이족" 증가로 인한 집에서 모든 걸 해결하는 홈코노미 관련 업종들의 성장세가 뚜렷했다.  
- NUGU의 경우 이러한 홈코노미 서비스를 제공하는데 굉장히 적합한 플랫폼이여, 개인별 적절한 여가활동 특히 영화라는 도메인을 기반으로한 맞춤형 서비스를 기획했다.  
...
## 유저 로그 기반 추천 서비스
![](./user_log_based.jpg)
- 유저 로그 정보 기반 - 현재는 proxy 서버의 유저 정보 db로 부터 추출 (현재는 특정 Persona에 대한 기록을 임의로 집어넣어서 추천을 진행해줍니다.)  
